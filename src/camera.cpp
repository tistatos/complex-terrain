...
Camera::Camera(const uint32_t width, const uint32_t height, const float fov)
		: mWidth(width), mHeight(height), mAspect((float)mWidth / mHeight), mFOV(fov), mNearPlane(1.0f),
			mFarPlane(500.0f), mDetachFrustum(false) {}

Camera::~Camera() { glDeleteBuffers(1, &mCameraUniformBuffer); }
...
void Camera::initialize() {
	...
	matrices.projection = glm::perspective(mFOV, mAspect, mNearPlane, mFarPlane);

	matrices.view =
			glm::lookAt(getPosition(), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));

	mCameraFrustum = Frustum(this);
	mCameraFrustumMesh = Mesh::createFrustum(&mCameraFrustum);
}

void Camera::setProjectionMatrix(glm::mat4 proj) { matrices.projection = proj; }

void Camera::update(double dt) {
	matrices.view = glm::lookAt(getPosition(), getPosition() + mFacing, glm::vec3(0.0f, 1.0f, 0.0f));
...
const CameraProperties Camera::getProperties() const {
	return CameraProperties{mFOV, mAspect, mNearPlane, mFarPlane};
}
