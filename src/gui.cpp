...
void GUI::loadCharacters() {
	...
	for (GLubyte c = 0; c < 128; c++) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, mFace->glyph->bitmap.width, mFace->glyph->bitmap.rows, 0,
		...
		Character character = {
				texture, glm::ivec2(mFace->glyph->bitmap.width, mFace->glyph->bitmap.rows),
				glm::ivec2(mFace->glyph->bitmap_left, mFace->glyph->bitmap_top), mFace->glyph->advance.x};
...
void GUI::renderText(std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color) {
	...
	glUniform3f(glGetUniformLocation(guiShader->getProgram(), "textColor"), color.x, color.y,
							color.z);
	glUniformMatrix4fv(glGetUniformLocation(guiShader->getProgram(), "projection"), 1, GL_FALSE,
										 &projection[0][0]);
	glBindVertexArray(mVAO);
